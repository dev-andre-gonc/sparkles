...
if %DEMO_ARG% == "demo" (
  echo Making Sparkles Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Sparkles Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild Sparkles-app.vcxproj /p:configuration=release /p:platform=x64

echo Building x64 binaries...
REM add projects with /t to build VST2 and AAX
msbuild Sparkles.sln /t:Sparkles-app;Sparkles-vst3;Sparkles-clap /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
msbuild Sparkles.sln /t:Sparkles-app;Sparkles-vst3;Sparkles-clap /p:configuration=release /p:platform=ARM64EC /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win-arm64ec.log;errorsonly
if %ERRORLEVEL% neq 0 (
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Sparkles.aaxplugin\Contents\Win32\Sparkles.aaxplugin --out .\build-win\aax\bin\Sparkles.aaxplugin\Contents\Win32\Sparkles.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Sparkles.aaxplugin\Contents\x64\Sparkles.aaxplugin --out .\build-win\aax\bin\Sparkles.aaxplugin\Contents\x64\Sparkles.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Sparkles.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Sparkles.iss"
  if %ERRORLEVEL% neq 0 (
  ...
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Sparkles Installer" ".\installer\Sparkles Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\Sparkles Installer.exe" ".\installer\Sparkles Demo Installer.exe"
  REM -del ".\installer\Sparkles Installer.exe"
  REM -)
