...
def main():
  ...
  vst3_bundle = build_dir + "\\Sparkles.vst3"
  ...
  if not zip:
    installer = "\\build-win\\installer\\Sparkles Installer.exe"

    if demo:
      installer = "\\build-win\\installer\\Sparkles Demo Installer.exe"

    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\build-win\\manual\\Sparkles manual.pdf"
    ]
    ...
    vst3_bundle = projectpath + "\\build-win\\Sparkles.vst3"
    if os.path.exists(vst3_bundle):
      add_folder_to_zip(zf, vst3_bundle, "Sparkles.vst3")

    # Add standalone executables
    files = [
      projectpath + "\\build-win\\Sparkles_x64.exe",
      projectpath + "\\build-win\\Sparkles_ARM64EC.exe",
    ]
    ...
    clap_files = [
      # Postbuild locations
      (projectpath + "\\build-win\\Sparkles_x64.clap", "Sparkles_x64.clap"),
      (projectpath + "\\build-win\\Sparkles_ARM64EC.clap", "Sparkles_ARM64EC.clap"),
      # Build output locations (fallback)
      (projectpath + "\\build-win\\clap\\x64\\Release\\Sparkles.clap", "Sparkles_x64.clap"),
      (projectpath + "\\build-win\\clap\\ARM64EC\\Release\\Sparkles.clap", "Sparkles_ARM64EC.clap"),
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Sparkles-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Sparkles-vst3_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Sparkles-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Sparkles-app_ARM64EC.pdb",
    projectpath + "\\build-win\\pdbs\\Sparkles-clap_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Sparkles-clap_ARM64EC.pdb"
  ]
