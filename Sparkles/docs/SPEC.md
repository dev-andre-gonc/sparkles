...
| `trigger_cooloff` | Minimum time (ms) after a trigger fires before another one is allowed to. A trigger arriving inside this window is **dropped silently** — same convention as the `confidence` drop above. Applies uniformly to both the audio and MIDI trigger paths, since both resolve through the same fire point. Default 100 ms. *(Implemented as `kParamTriggerCooloff`, enforced in `Sparkles::FireSprinkle`.)* |
