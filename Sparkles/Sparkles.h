...
enum ECtrlTags
{
  ...
  // kParamGroups' clusters on those tabs (params whose change only takes effect on the next
  // sprinkle -- see mPendingNotes and CLAUDE.md). Visible on 3 tabs, not just 1, so it isn't wired
  // through the single-tab setTabbed helper -- see mLayoutFunc.
  kCtrlTagFootnote,
...
private:
  ...
  // Notes generated by FireSprinkle whose structure (pitch/timing) is resolved but whose
  // properties (velocity/pan/duration/ADSR) deliberately aren't yet -- ProcessBlock resolves each
  // one's properties against that block's own live snapshot, right before it's actually due, so a
  // knob twisted mid-sprinkle affects rays/sparkles that haven't fired yet. See
  // core/PendingNoteQueue.h and core/SparkleGenerator.h's ResolveEventProperties. Must be cleared
  // in lockstep with mEventScheduler/mSynthEngine on Shut Up (see ShutUp()) -- otherwise a note
  // still waiting here would survive a panic and fire later anyway.
  sparkle_core::PendingNoteQueue<> mPendingNotes;
